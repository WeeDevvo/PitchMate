...
/// <summary>
/// Permanently removes every <see cref="InAppNotification"/> addressed to an anonymised squad membership,
/// regardless of read state (Requirement 11.2). It is invoked from the squads-and-membership erasure path
/// (task 11.1), mirroring the way the squad <c>OnUserErasedHandler</c> is wired, and never touches any
/// match record or rating-replay input (Requirement 11.6).
/// <para>
/// The handler stages the removals through <see cref="INotificationRepository.RemoveForMembershipAsync"/>
...
/// </para>
/// </summary>
public sealed class RemoveNotificationsForMembershipHandler
{
    ...
    /// <param name="notifications">The notification persistence surface used to stage the anonymised membership's removals.</param>
    /// <param name="unitOfWork">The unit of work that commits the removals atomically.</param>
    public RemoveNotificationsForMembershipHandler(INotificationRepository notifications, IUnitOfWork unitOfWork)
    {
    ...
    /// <summary>
    /// Permanently removes every in-app notification addressed to <paramref name="membershipId"/>,
    /// committing the removal atomically. Returns success — including when the scope was empty — or
    ...
    /// </summary>
    /// <param name="membershipId">The anonymised membership whose notifications are removed.</param>
    /// <param name="cancellationToken">A token to cancel the operation.</param>
    public async Task<Result> HandleAsync(Guid membershipId, CancellationToken cancellationToken)
    {
        try
        {
            await _notifications.RemoveForMembershipAsync(membershipId, cancellationToken);
        ...
        catch (Exception ex) when (ex is not OutOfMemoryException and not StackOverflowException)
        {
            ...
            return Result.Fail(new Domain.Notifications.NotificationError(
                ...
                "Failed to remove the anonymised membership's in-app notifications."));
        }
